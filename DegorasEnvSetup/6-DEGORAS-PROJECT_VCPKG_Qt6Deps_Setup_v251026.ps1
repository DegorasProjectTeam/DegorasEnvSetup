# ====================================================================
# DEGORAS-PROJECT VCPKG QT6 DEPS SETUP SCRIPT
# --------------------------------------------------------------------
...
$globalLogFile = Join-Path $logsDir "${timestamp}_vcpkg-Qt6deps-setup.log"
...
# --------------------------------------------------------------------
# STEP 1.5 – Create temporary JOM shim (redirects to mingw32-make)
# --------------------------------------------------------------------

Write-Info "STEP 1.5: Creating temporary JOM shim..."

$jomToolDir = "${devDrive}\vcpkg\downloads\tools\jom\jom-1_1_4"
$jomExe     = Join-Path $jomToolDir "jom.exe"
$origBackup = Join-Path $jomToolDir "jom.exe.bak"
$mingwMake  = "${devDrive}\msys64\ucrt64\bin\mingw32-make.exe"

if (-not (Test-Path $jomToolDir)) { New-Item -ItemType Directory -Path $jomToolDir | Out-Null }

# back up any existing jom
if (Test-Path $jomExe) { Rename-Item $jomExe $origBackup -Force }

Copy-Item $mingwMake $jomExe -Force
Write-Info "Fake jom.exe shim created at $jomExe"

# STEP 2: Install Qt6 libraries
# --------------------------------------------------------------------

Write-Info "STEP 2: Install required Qt6 deps packages with triplet 'x64-mingw-dynamic-degoras'"
...
# --------------------------------------------------------------------
# STEP 2.5 – Restore original JOM if it existed
# --------------------------------------------------------------------

Write-Info "STEP 2.5: Restoring original JOM state..."

if (Test-Path $origBackup) {
    Remove-Item $jomExe -Force
    Rename-Item $origBackup $jomExe
    Write-Info "Original jom.exe restored."
} else {
    Remove-Item $jomExe -Force
    Write-Info "Removed temporary shim."
}
...
# Final logs.
Write-Info "DEGORAS-PROJECT VCPKG Qt6 deps setup completed successfully."
